...
def calcTermFreqAcrossArtists(artists, terms):
    ...
    term_freqs = pd.concat(
        [calcFreqOfTerm(artist, terms) for artist in artists], axis=0
    )
...
songs["release_date"] = [
    datetime.strptime(x, "%Y-%m-%d") if x is not None else None
    for x in songs["release_date"]
]

songs["age"] = [
    (datetime.now() - x).days / 365.25 if x is not None else None
    for x in songs["release_date"]
]
...
nb_songs_per_artist_l = [
    songs.groupby("artist").count().loc[artist, "title"] for artist in artists
]
nb_songs_per_artist = pd.DataFrame(
    nb_songs_per_artist_l, columns=["Number of songs"], index=artists
)
nb_songs_per_artist_per_year = (
    nb_songs_per_artist["Number of songs"] / songs.groupby("artist")["age"].max().values
)
...
nb_words_per_song_per_artist = pd.Series(
    [
        calc_artist_nb_of_words(artist)
        / nb_songs_per_artist.loc[artist, "Number of songs"]
...
fig1 = px.bar(
    x=artists,
    y=nb_songs_per_artist_l,
    title="Distribution of number of songs per artist",
    labels={"x": "Artists", "y": "Number of songs"},
    width=800,
    height=800,
)
...
fig2 = px.bar(
    ...
    title="Distribution of number of songs per artist per year",
    labels={"x": "Artists", "y": "Number of songs per year"},
    width=800,
    height=800,
)
...
fig3_1 = px.scatter(
    ...
    y=y,
    hover_name=artists,
    labels={"x": "Years of activity", "y": "Number of songs per year"},
    width=800,
    height=800,
)
...
fig3.update_layout(
    {
        "title": "Number of songs per year per years of activity",
        "width": 800,
        "height": 800,
    }
)
...
fig4 = px.histogram(
    word_counts,
    title="Histogram of words per song",
    width=800,
    height=800,
    labels={"value": "Words in song", "count": "Number of songs"},
)

fig5 = px.bar(
    x=artists,
    y=nb_words_per_song_per_artist,
    width=800,
    height=800,
    title="Distribution of number of words per song per artist",
    labels={"x": "Artists", "y": "Number of words"},
)

fig6 = px.histogram(
    lexical_diversity["mean"] * 100,
    width=800,
    height=800,
    title="Histogram of percentage of UNIQUE words per song",
    labels={"x": "Percentage of unique words"},
)

fig7 = px.bar(
    x=artists,
    y=lexical_diversity["mean"] * 100,
    width=800,
    height=800,
    title="Percentage of UNIQUE words per song per artist",
