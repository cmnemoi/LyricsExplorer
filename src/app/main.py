...
def show_artist_page():
    ...
    st.subheader(selected_artist+' produced '+str(back.nb_songs_per_artist.loc[selected_artist,"Number of songs"])
        +' songs in total and '+str('{:0.2f}'.format(back.nb_songs_per_artist_per_year.loc[selected_artist])   )+ ' songs per year')
    ...
    wc = back.WordCloud(width=800,height=400).generate_from_frequencies(back.gen_freq(back.artist_lyrics(selected_artist)))
